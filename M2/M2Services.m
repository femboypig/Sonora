...
static NSString * const kPlaybackHistoryDefaultsKey = @"m2_playback_history_v1";
...
- (void)startCurrentTrack {
    ...
    [M2PlaybackHistoryStore.sharedStore recordTrackID:track.identifier];
...
@interface M2PlaybackHistoryStore ()

@property (nonatomic, strong) NSMutableArray<NSString *> *trackIDs;

@end

@implementation M2PlaybackHistoryStore

+ (instancetype)sharedStore {
    static M2PlaybackHistoryStore *store;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        store = [[M2PlaybackHistoryStore alloc] init];
    });
    return store;
}
...
- (NSArray<M2Track *> *)recentTracksWithLibrary:(M2LibraryManager *)library
    ...
    NSMutableArray<M2Track *> *tracks = [NSMutableArray arrayWithCapacity:trackIDs.count];
    for (NSString *trackID in trackIDs) {
        M2Track *track = [library trackForIdentifier:trackID];
        if (track != nil) {
            [tracks addObject:track];
